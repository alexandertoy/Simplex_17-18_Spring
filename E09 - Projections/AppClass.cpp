...
void Application::Display(void)
{
	...
	case 3:
		m_pCamera->ResetCamera();
		//m_pCamera->SetPerspective(false);
		m_pCamera->SetPositionTargetAndUp(vector3(0.0f, 45.0f, 1.0f), vector3(0.0f, 0.0f, 00.0f), AXIS_Y);
		break;
	case 4:
		m_pCamera->ResetCamera();
		//m_pCamera->SetPerspective(false);
		m_pCamera->SetPositionTargetAndUp(AXIS_Z * -15.0f, ZERO_V3, AXIS_Y);
		break;
	case 5:
		m_pCamera->ResetCamera();
		m_pCamera->SetPerspective(false);
		m_pCamera->SetPositionTargetAndUp(AXIS_Z * -10.0f, ZERO_V3, AXIS_Y);
		break;
	case 6:
		m_pCamera->ResetCamera();
		m_pCamera->SetPerspective(false);
		m_pCamera->SetPositionTargetAndUp(AXIS_Z / 200.0f, ZERO_V3, AXIS_Y);
		break;
	case 7:
		m_pCamera->ResetCamera();
		m_pCamera->SetPositionTargetAndUp(AXIS_Z * 10.0f, ZERO_V3, AXIS_Y * -1.0f);
		break;
	}
